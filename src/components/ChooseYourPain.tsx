        ...
        <div className="relative">
          ...
          <div className="overflow-hidden px-1" ref={emblaRef}>
            <div className="flex" style={{ transitionTimingFunction: "ease", transitionDuration: "0.4s" }}>
              {solutions.map((sol, i) => (
                <div key={sol.slug} className="min-w-0 shrink-0 grow-0 basis-full sm:basis-1/2 lg:basis-1/3 pl-4 first:pl-0">
                  <motion.button
                    initial={{ opacity: 0, y: 30 }}
                    animate={isInView ? { opacity: 1, y: 0 } : {}}
                  ...
                  >
                    <div className="h-48 overflow-hidden relative">
                      <img src={sol.image} alt={sol.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" loading="lazy" />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/30 to-transparent" />
                    </div>
                    <div className="p-5">
                      <div className="flex flex-wrap gap-2 mb-3">
                        {sol.tags.map((tag) => (
                          <span key={tag} className="text-[11px] font-semibold uppercase tracking-wider text-primary bg-primary/10 px-2.5 py-1 rounded-md">{tag}</span>
                        ))}
                      </div>
                      <h3 className="font-bold text-foreground leading-snug group-hover:text-primary transition-colors">{sol.title}</h3>
                    </div>
                  </motion.button>
                </div>
              ))}
