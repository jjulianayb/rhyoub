...
const BeeAvatar = ({ className = "" }: { className?: string }) => <img className={`bee-avatar-image ${className}`} src={beeAvatar} alt="Bee, IA Mentora" />;
...
const VisualRedesign = () => {
  ...
  return (
    <main className="redesign-page">
      ...
      <section className="platform-section" id="plataforma" aria-labelledby="platform-title"><div className="container"><div className="section-heading split-heading"><div><span className="section-kicker">DA ESTRATÉGIA À PRÁTICA</span><h2 id="platform-title">Uma plataforma completa para o <em>ciclo de desenvolvimento.</em></h2></div><div><p>Da leitura do contexto à próxima ação, a youB integra dados, desenvolvimento e inteligência em uma experiência contínua.</p><Link to="/plataforma" className="text-link">Conheça todas as soluções <ArrowRight size={15} /></Link></div></div><div className="capability-grid">{capabilities.map((capability, index) => { const Icon = capability.icon; const isActive = activeCapability?.title === capability.title; return <button className={`capability-card${isActive ? " is-active" : ""}`} type="button" key={capability.title} aria-expanded={isActive} onClick={() => setActiveCapability(isActive ? null : capability)} style={{ animationDelay: `${index * 70}ms` }}><div className="capability-icon">{capability.title === "IA Mentora Bee" ? <BeeAvatar /> : <Icon size={20} />}</div><h3>{capability.title}</h3><p>{capability.text}</p><span className="capability-open-label">{isActive ? "Fechar detalhe" : "Abrir detalhe"}<ChevronRight size={14} /></span><span className="card-arrow"><ArrowRight size={15} /></span></button>; })}</div>{activeCapability && <div className="capability-dialog-backdrop" role="presentation" onClick={() => setActiveCapability(null)}><div className="capability-dialog" role="dialog" aria-modal="true" aria-labelledby="capability-dialog-title" onClick={(event) => event.stopPropagation()}><button className="capability-dialog-close" type="button" aria-label="Fechar detalhe" onClick={() => setActiveCapability(null)}><X size={18} /></button><div className="capability-icon">{activeCapability.title === "IA Mentora Bee" ? <BeeAvatar /> : ActiveCapabilityIcon && <ActiveCapabilityIcon size={20} />}</div><span className="section-kicker">Na plataforma youB</span><h3 id="capability-dialog-title">{activeCapability.title}</h3><p>{activeCapability.text}</p><Link to="/plataforma" className="button button-dark" onClick={() => setActiveCapability(null)}>Conhecer a plataforma <ArrowRight size={16} /></Link></div></div>}</div></section>

      <section className="narrative-section"><div className="container narrative-grid"><div className="narrative-visual"><div className="narrative-label"><Sparkles size={15} /> Produto que transforma contexto em ação</div><div className="signal-card"><div className="signal-header"><span className="signal-icon"><BarChart3 size={16} /></span><div><strong>Leitura de desenvolvimento</strong><small>Uma visão para orientar a conversa</small></div></div><div className="signal-bars"><span style={{ width: "42%" }} /><span style={{ width: "67%" }} /><span style={{ width: "54%" }} /><span style={{ width: "82%" }} /><span style={{ width: "72%" }} /></div><div className="signal-footer"><span>Contexto organizado</span><span className="signal-positive">Próximo passo identificado</span></div></div><div className="action-card"><div className="bee-mini"><BeeAvatar /></div><div><span className="mini-label">BEE · IA MENTORA</span><strong>Comece pela conversa certa.</strong><p>Recomendações para apoiar líderes e colaboradores no desenvolvimento contínuo.</p></div><ArrowRight size={17} /></div></div><div className="narrative-copy"><span className="section-kicker">PROBLEMA → INSIGHT → AÇÃO</span><h2>Menos planilhas soltas. Mais continuidade para desenvolver pessoas.</h2><p>A plataforma organiza sinais, conversas e planos de desenvolvimento para que RH e lideranças avancem com clareza — sem perder o contexto humano.</p><ul><li><CheckCircle2 size={17} /> Decida com dados e escuta.</li><li><CheckCircle2 size={17} /> Conecte avaliação, feedback e PDI.</li><li><CheckCircle2 size={17} /> Acompanhe a evolução de cada jornada.</li></ul><Link to="/plataforma" className="button button-dark">Ver a plataforma <ArrowRight size={16} /></Link></div></div></section>
      ...
      <section className="final-cta"><div className="container final-cta-inner"><div><span className="section-kicker">VAMOS CONVERSAR?</span><h2>Veja a <em>youB</em> funcionando na sua empresa.</h2><p>Conheça como dados, desenvolvimento e inteligência se conectam para transformar decisões em evolução.</p><a className="button button-primary" href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer">Agendar demonstração <ArrowRight size={16} /></a></div><div className="final-bee-visual"><div className="final-glow" /><div className="final-bee-card"><div className="bee-avatar large"><BeeAvatar /></div><span className="mini-label">BEE · IA MENTORA</span><strong>A evolução das pessoas começa com uma boa conversa.</strong><span className="bee-pulse" /></div></div></div></section>
